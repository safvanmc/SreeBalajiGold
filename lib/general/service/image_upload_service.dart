...
class ImageUploadServices {
  static FutureResult<List<String>> uploadListImageStorage({
    ...
    try {
      ...
      return left(
        MainFailure.serverFailure(msg: e.toString()),
      );
    } catch (e) {
  ...
  static Future<void> deleteFirebaseStorageFile(
      BuildContext context, List<String> imageUrlList) async {
    ...
    try {
      ...
      for (final url in imageUrlList) {
        functionList.add(_checkAndDeleteImage(context, url, firebaseStorage));
      }
